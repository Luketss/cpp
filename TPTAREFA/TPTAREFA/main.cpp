...
typedef unsigned (WINAPI* CAST_FUNCTION)(void*);
...
HANDLE analise_supervisor_toggle_event;
...
DWORD WINAPI leitura_medicao(void*);
...
int main()
{
    ...
    STARTUPINFO si_analise_supervisor;
    ...
    ZeroMemory(&si_analise_supervisor, sizeof(si_analise_supervisor));
    si_analise_supervisor.cb = sizeof(si_analise_supervisor);	// Tamanho da estrutura em bytes
    ...
    analise_supervisor_toggle_event = CreateEvent(NULL, FALSE, FALSE, TEXT("analise_supervisor_toggle_event"));
    ...
    /* Cria processo de análise de supervisor */

    status = CreateProcess(
        L"..\\x64\\Debug\\Supervisor.exe",
        NULL,
        NULL,
        NULL,
        FALSE,
    ...
    CloseHandle(analise_supervisor_toggle_event);
...
DWORD WINAPI leitura_medicao(void* id)
{
    ...
    do {
        ...
        Message message;
        message.supervisor = generate_message_sup(message_counter);
...
DWORD WINAPI leitura_dados(void* id)
{
    ...
    do {
        ...
        Message message;
        message.pcp = generate_message_pcp(message_counter);
        message.type = 2;
...
DWORD WINAPI captura_mensagens(void* id)
{
